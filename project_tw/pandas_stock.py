...
lasting_yrs = 3
std_grd = 20
df = pd.read_csv('stock_list_s2006e2025_unfiltered.csv')

#drop column bah and bal
drop_roi_type = ['bah', 'bal']
drop_roi_cl = ['s'+'2006'+'e'+str(ey)+ roit for ey in range(2007, 2026) for roit in drop_roi_type]
df2 = df.drop(drop_roi_cl, axis=1)

#drop column unused yrs
drop_yrs_cl = ['s'+'2006'+'e'+str(ey)+'yrs' for ey in range(2007, 2025)]
...
#sorted by 's2006e2025bao' , TBD with variable
df5=df4.sort_values(by='s2006e2025bao', ascending=False)
...
df8.to_csv('stock_list_s2006e2025_filtered.csv')

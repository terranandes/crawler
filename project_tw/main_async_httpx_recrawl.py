...
if __name__ == '__main__':
    help_msg ='\
Usage: main_async_httpx_recrawl.py [start_year] [end_year]\n\
For example:\n\
main_async_httpx_recrawl.py 2010 2026\n\
Only crawls the stock codes listed in recrawl_codes.txt\
'
    start_year_in = 2010
    end_year_in   = 2026
    ...
    print(f'Running main_async_httpx_recrawl_s{start_year_in}e{end_year_in}')
    ...
    #Get target stock list from recrawl file (one code per line)
    fn_recrawl = 'recrawl_codes.txt'
    with open(fn_recrawl, 'r', encoding='utf-8') as recrawlFile :
        stock_list = [line.strip() for line in recrawlFile if line.strip()]
    print(f'Loaded {len(stock_list)} codes from {fn_recrawl}')
    #print(stock_list)

    #Get expansion rate of each stock
    fn_json = 'stock_list_recrawl.json'
    #try:
